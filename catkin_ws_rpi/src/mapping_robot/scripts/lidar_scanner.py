...
if __name__ == '__main__':
	...
	progress_pub = rospy.Publisher('progress_map3D', Int16, queue_size=10)
	...
	wiringpi.pwmWrite(horizontalServo, 470)
        wiringpi.pwmWrite(verticalServo, 490)
	rospy.sleep(0.4)

	wiringpi.pwmWrite(horizontalServo, 98)
        wiringpi.pwmWrite(verticalServo, 118)
	rospy.sleep(0.4)
	...
	while not rospy.is_shutdown():
		...
		if scan3DAngle == 0:
			...
			for i in range(num_readings2D):
				...
				wiringpi.pwmWrite(horizontalServo, 98+(i*186/180))
			wiringpi.pwmWrite(horizontalServo, 98)
			wiringpi.pwmWrite(verticalServo, 490)
			rospy.sleep(0.4)
			...
			for i in range(scan3DAngle):
				...
				if i < 45:
					...
					for j in range(num_readings3D):
						...
						# Move to the servo to the next point
						wiringpi.pwmWrite(horizontalServo, 98+(j*372/num_readings3D))

					wiringpi.pwmWrite(horizontalServo, 98)
					wiringpi.pwmWrite(verticalServo, 490 - (i * 372/num_readings3D))
					rospy.sleep(0.4)
					...
					wiringpi.pwmWrite(horizontalServo, 98)
					wiringpi.pwmWrite(verticalServo, 118 + (i * 372/num_readings3D))
					...
					if rospy.is_shutdown():
                                        ...
                                        for j in range(num_readings3D/2):
                                                ...
                                                if len(serial_data) == 4:
                                                        ...
                                                        if distance < 12:
                                                                point = Point32()
                                                                point.x = distance * math.cos(math.radians(i*(num_readings3D/180))) * math.cos(math.radians(2*j*num_readings3D/180 - 90))
                                                                point.y = distance * math.cos(math.radians(i*(num_readings3D/180))) * math.sin(math.radians(2*j*num_readings3D/180 - 90))
                                                                point.z = distance * math.sin(math.radians(i*(num_readings3D/180)))
                                                                scan3D.points.append(point)
                                                                #scan3D.channels[0].values.append(distance)
                                                                distance_channel.values.append(distance)
                                                ...
                                                # Move to the servo to the next point
                                                wiringpi.pwmWrite(horizontalServo, 98+(2*j*372/num_readings3D))
